...
<body class="theme-cyan">
    ...
    $(document).ready(function() {
        ...
        const clinicTable = $('#clinicTable').DataTable();
        ...
        $('#clinicForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    const imagePath = '/' + clinic.image;

                    const actionButtons = `
                    <button type="button" class="btn btn-sm btn-primary edit-clinic"
                        data-id="${clinic.id}"
                        data-title="${clinic.title}"
                        data-image="${imagePath}">
                        Edit
                    </button>
                    <button type="button" class="btn btn-sm btn-danger delete-clinic"
                        data-id="${clinic.id}">
                        Delete
                    </button>
                `;
                    ...
                    if (isEdit) {
                        const row = clinicTable.row($(
                            `#clinicTable tr[data-id="${clinic.id}"]`));
                        row.data([
                            clinic.id,
                        ...
                        const newRow = clinicTable.row.add([
                            clinic.id,
                            `<img src="${imagePath}" width="80">`,
                            clinic.title,
                            actionButtons
                        ]).draw(false).node();
                        $(newRow).attr('data-id', clinic.id);
                    }
                    ...
                    $('#clinicForm button[type="submit"]').text('Submit');

                    Swal.fire('Success', response.message || 'Clinic saved!', 'success');
                },
                error: function(xhr) {
        ...
        $(document).on('click', '.edit-clinic', function() {
            ...
            $('#clinicForm button[type="submit"]').text('Update');
        });
        $(document).on('click', '.delete-clinic', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'This will permanently delete the clinic.',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                $('#clinicTable').DataTable().row($(
                                    `tr[data-id="${id}"]`)).remove().draw(false);
                        ...
                        error: function() {
                            Swal.fire('Error', 'An error occurred during deletion.',
                                'error');
                        }
                    });
                }
